...
func (s *congestionControl) onPacketSent(sentBytes uint64, sentTime time.Time) {
	if s.enableCubic {
...
func (s *congestionControl) onPacketAcked(sentBytes uint64, sentTime time.Time, rtt time.Duration, now time.Time) {
...
func (s *congestionControl) onPacketDiscarded(sentBytes uint64) {
	if s.state.bytesInFlight > sentBytes {
		s.state.bytesInFlight -= sentBytes
	} else {
		s.state.bytesInFlight = 0
...
func (s *congestionControl) renoOnPacketAcked(sentBytes uint64) {
...
type proportionalRateReduction struct {
	...
	sndCnt     uint64 // Bytes should be sent (sndcnt).
}
