...
func TestDecryptClientInitial(t *testing.T) {
	const clientCID = `8394c8f03e515708`
	const clientInitial = `
	...
	client, _ := newInitialSecrets(testdata.DecodeHex(clientCID))
...
func TestDecryptServerInitial(t *testing.T) {
	const serverCID = `8394c8f03e515708`
	const serverInitial = `
	...
	_, server := newInitialSecrets(testdata.DecodeHex(serverCID))
...
func TestComputeRetryIntegrity(t *testing.T) {
	...
	retry := testdata.DecodeHex(`ff000000010008f067a5502a4262b5746f6b656e`)
	b := make([]byte, 0, 128)
